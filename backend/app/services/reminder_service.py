...
def get_today_reminders(
    ...
    target_date = date if date else date_module.today().isoformat()
...
def create_session_log(
    ...
    logger.info(
        ...
        session.id, user.id, canal, destinatario, status, tipo, reminder_offset_minutes, error_message
    )
...
def send_session_reminder(
    ...
    logger.info(
        ...
        session_id, user.id, tipo, reminder_offset_minutes
    )

    session = get_session_by_id_and_user(db, session_id, user.id)
    ...
    logger.info(
        ...
        session.id, user.id, session.cliente_nome, bool(session.cliente_email), bool(user.email)
    )
    ...
    if (
        ...
        logger.info(
            ...
            session.id, user.id, reminder_offset_minutes
        )
        return {
            "status": "skipped",
    ...
    if session.cliente_email:
        try:
            ...
            create_session_log(
                db=db, session=session, user=user,
                canal="email_cliente", destinatario=session.cliente_email,
                status="sent", tipo=tipo,
                reminder_offset_minutes=reminder_offset_minutes, sent_at=sent_at
            )
            logger.info("REMINDER_CLIENT_EMAIL_SENT session_id=%s to=%s", session.id, session.cliente_email)
        except Exception as error:
            logger.exception("REMINDER_CLIENT_EMAIL_ERROR session_id=%s to=%s error=%s", session.id, session.cliente_email, str(error))
            create_session_log(
                db=db, session=session, user=user,
                canal="email_cliente", destinatario=session.cliente_email,
        ...
        logger.warning("REMINDER_CLIENT_EMAIL_SKIPPED_NO_EMAIL session_id=%s cliente=%s", session.id, session.cliente_nome)
    ...
    if user.email:
        try:
            ...
            create_session_log(
                db=db, session=session, user=user,
                canal="email_usuario", destinatario=user.email,
                status="sent", tipo=tipo,
                reminder_offset_minutes=reminder_offset_minutes, sent_at=sent_at
            )
            logger.info("REMINDER_USER_EMAIL_SENT session_id=%s to=%s", session.id, user.email)
        except Exception as error:
            logger.exception("REMINDER_USER_EMAIL_ERROR session_id=%s to=%s error=%s", session.id, user.email, str(error))
            create_session_log(
                db=db, session=session, user=user,
                canal="email_usuario", destinatario=user.email,
                status="error", tipo=tipo,
        ...
        logger.warning("REMINDER_USER_EMAIL_SKIPPED_NO_EMAIL session_id=%s user_id=%s", session.id, user.id)

    logger.info("REMINDER_SEND_DONE session_id=%s user_id=%s tipo=%s offset=%s", session.id, user.id, tipo, reminder_offset_minutes)
...
def process_automatic_reminders_for_user(
    ...
    logger.info("REMINDER_AUTO_SCAN_USER_START user_id=%s today=%s now=%s", user.id, today, current_time.isoformat())

    sessions = list_today_sessions(db=db, user_id=user.id, today=today)

    logger.info("REMINDER_AUTO_SCAN_USER_SESSIONS user_id=%s count=%s", user.id, len(sessions))
    ...
    for session in sessions:
        ...
        for offset_minutes in AUTOMATIC_OFFSETS_MINUTES:
            reminder_time = session_datetime - timedelta(minutes=offset_minutes)
            window_start = reminder_time
            window_end = reminder_time + timedelta(minutes=REMINDER_WINDOW_MINUTES)

            logger.info(
                "REMINDER_AUTO_CHECK session_id=%s offset=%s now=%s window_start=%s window_end=%s",
                session.id, offset_minutes, current_time.isoformat(), window_start.isoformat(), window_end.isoformat()
            )
            ...
            result = send_session_reminder(
                ...
                tipo="automatico", reminder_offset_minutes=offset_minutes
            )
            processed.append(result)

    logger.info("REMINDER_AUTO_SCAN_USER_DONE user_id=%s processed=%s", user.id, len(processed))
...
def get_reminder_logs(
    ...
    return list_reminder_logs_by_user(db, user_id)
